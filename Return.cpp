...
def return_db():
    ...
    try:
        checkavailability=" select * from books where available='NO' AND bid='"+bid+"';"
...
def returnBooks():
    ...
    greet = Label(window, font = ('arial', 30, 'bold'), fg="red",text = "   Return Books")
    greet.grid(row = 0,columnspan = 223)
